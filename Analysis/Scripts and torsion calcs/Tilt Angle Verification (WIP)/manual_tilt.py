...
def assign_leaflets(atom_group):
    ...
    # calculates z-coord using center of mass of individual lipids
    com_z_values = [res.atoms.center_of_mass()[2] for res in atom_group.residues]

    # get average z-coord to use as the cutoff
...
def calculate_tilt_angle(residue, phosphorus_atom='P', nitrogen_atom='N'): # tilt calculation with constant axis
    ...
    # get angle between the PN vector and the z-axis
...
def read_sim(topology, trajectory, phosphorus_atom='P', nitrogen_atom='N'): # for trj read/avg
    ...
    # calls for avg tilt of each leaflet
